...
const DataRouter = require('./Server/Api/Data/dataApiRoutes')
...
if (process.env.NODE_ENV === 'development') {
    ...
    httpsServer = https.createServer({
        key: fs.readFileSync('/usr/share/ca-certificates/privkey.pem'),
        cert: fs.readFileSync('/usr/share/ca-certificates/cert.pem'),
    }, app);
    app.use(forceSSL);
}
...
httpsServer.listen(port, async () => {
    ...
    console.log(`HTTPS Server running on addr ${process.env.HOST} port ${port}`);
